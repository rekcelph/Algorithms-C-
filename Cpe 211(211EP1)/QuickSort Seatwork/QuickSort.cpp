...
// Use to chop things to seperate 
...
void quicksort(int arr[], int low, int high) {
    if (low < high) {
        int p = partition(arr, low, high);//pakening pivot element shit 

        quicksort(arr, low, p - 1);//left sub array less pivot
        quicksort(arr, p + 1, high);//right sub array greater than pivot
    }
}
